<?php
	...
	/* Push Huawei */
	include_once (dirname(__FILE__) . '/huawei-push/src/example/push_common/test_sample_push_msg_common.php');
	include_once (dirname(__FILE__) . '/huawei-push/src/push_admin/Constants.php');
	...
	class Api extends Rest{
		...
		public function login(){
			...
			$huawei_device = $this->param['huawei_device'];
			...
			try {
				...
				if (!is_array($usuario)) {
					...
					if(!$huawei_device){
						...
						$query = "UPDATE LOC_PERSONA SET TOKEN = '$token', UPDATED_AT = SYSDATE, TOKEN_HUAWEI = NULL WHERE ID_PERSONA = $id_persona";

					}else{

						$query = "UPDATE LOC_PERSONA SET TOKEN = NULL, UPDATED_AT = SYSDATE, TOKEN_HUAWEI = '$token' WHERE ID_PERSONA = $id_persona";
		...
		public function getActivesProtocols2(){
			...
			try {
				...
				$query = "	SELECT T1.ID_ORGANIZACION, T1.NOMBRE, T2.ADMINISTRADOR
							FROM LOC_ORGANIZACION T1
							INNER JOIN LOC_PERSONA_ORGANIZACION T2
							ON T1.ID_ORGANIZACION = T2.ID_ORGANIZACION
							WHERE T2.ID_PERSONA = $userID";
		...
		public function getActivesProtocols3(){
			...
			try {
				...
				while ($data = oci_fetch_array($stid, OCI_ASSOC)) {
					...
					while ($data2 = oci_fetch_array($stid2,OCI_ASSOC)) {
						...
						$data2["ADMINISTRADOR"] = $data["ADMINISTRADOR"];
						$protocolos [] = $data2;
		...
		public function testHuawei(){
			...
			$testToken = "IQAAAACy0cW-AABbROTLHSLng-J6D2nC2XQitx9Ivm6OuSXsb5qUkCRLFoKtY4R-oxCrSdefiyp_IwTfqq84Lv_kSkSuoSUbXLRgc4KPIr75eaSr2A";
			...
			foreach ($message_arr as $message) {
				$message=str_ireplace("*push_token*", '"'.$testToken.'"',$message);
